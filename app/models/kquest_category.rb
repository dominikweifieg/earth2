class KquestCategory < ActiveRecord::Base
  ...
  def self.import_all
    ...
    categories.each do |lc| 
      ...
      category = Category.find_by_old_uid_and_old_type(lc.cid, 'cat')
      if(category)
        # category.questions.clear
        old_time_stamp = category.updated_at.to_i
        category.touch
        puts "Found #{lc.cat_title}"
      else
        puts "Create new category for #{lc.cat_title}"
        category = Category.new(:title => lc.cat_title, :short_title => lc.cat_title, :description => "", 
          :old_uid => lc.cid, :old_type => 'cat', :identifier => "de.kreawi.mobile.#{lc.cat_title.parameterize('_')}".sub(/-/, "_"), :app_name => "", :is_iap => false)
      end
    
      lc.kquest_questions.each do |legacy_question|
        next if legacy_question.deleted == 1
        question = Question.find_by_old_uid_and_old_type(legacy_question.uid, 'cat')
        if question
