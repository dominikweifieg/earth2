class KquestQuestionSetsController < ApplicationController
  ...
  def import(legacy_category, reimport)
    if(reimport)
      category = Category.find_by_old_uid_and_old_type(@category.cid, 'set')
      category.questions.clear
      category.touch
    else
      category = Category.new(:title => legacy_category.set_name, :description => legacy_category.set_name, 
        :old_uid => legacy_category.set_id, :old_type => 'set' , :identifier => "de.kreawi.mobile.#{legacy_category.set_name.parameterize('_')}".sub(/-/, "_"))
    end
