class KquestQuestionSetsController < ApplicationController
  ...
  def show
    ...
    @imported = Category.exists?(['old_uid = ? AND old_type = "set"', params[:id]])
    @earth_category = Category.find_by_old_uid_and_old_type(params[:id], 'set')
  ...
  def import(legacy_category, reimport)
    if(reimport)
      category = Category.find_by_old_uid_and_old_type(@category.set_id, 'set')
      # category.questions.clear
      category.touch
    else
      category = Category.new(:title => legacy_category.set_name, :short_title => legacy_category.set_name, :description => legacy_category.set_name, 
